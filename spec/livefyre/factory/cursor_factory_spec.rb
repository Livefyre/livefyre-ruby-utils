...
describe Livefyre::CursorFactory do
  ...
  it 'should create a cursor with a personal stream resource' do
    topic = Livefyre::Topic.create(@network, 'ID', 'LABEL')
    cursor = Livefyre::CursorFactory.get_personal_stream_cursor(@network, USER_ID)
    expect(cursor.data.resource).to eq("#{@network.get_urn_for_user(USER_ID)}:personalStream")

    cursor = Livefyre::CursorFactory.get_personal_stream_cursor(@network, USER_ID, 10, Time.new)
  ...
  it 'should create a cursor with a topic stream resource' do
    ...
    cursor = Livefyre::CursorFactory.get_topic_stream_cursor(@network, topic, 10, Time.new)
    expect(cursor.data.limit).to eq(10)
  end
end